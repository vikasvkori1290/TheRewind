...
def main() -> None:
    ...
    parser.add_argument("--rewind-limit", type=int, default=None,
                        help="also run Rewind with this smaller context limit (label rewind@N)")
    ...
    variants = [Variant.of(s) for s in strategies]
    ...
    for row in summarize(results):
        print(f"{row['strategy']:>12}: accuracy {row['accuracy']} · tokens {row['total_tokens']} "
