...
from rewind.recall import RECALL_SYSTEM, RecallTool
from rewind.session import DEFAULT_SYSTEM, Session
...
def build_session(strategy: str, settings: Settings, llm: LLM,
                  archive: ArchiveStore | None = None, session_id: str | None = None) -> Session:
    sid = session_id or uuid.uuid4().hex[:12]
    common = dict(session_id=sid, llm=llm, context_limit=settings.context_limit,
